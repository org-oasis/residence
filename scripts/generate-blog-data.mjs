...
/** One module per article and per language, so a reader downloads only theirs. */
const CONTENT_DIR = join(ROOT, "src", "lib", "blog-content");
...
/**
 * Article metadata only. The body and the FAQ live in one module per article
 * and per language under src/lib/blog-content/, loaded on demand by the route:
 * keeping all 183 bodies in this file shipped 2.9 MB of JavaScript to every
 * reader, on every blog page.
 */
...
const index = articles.map((article) => {
  const meta = { ...article };
  delete meta.bodyHtml;
  delete meta.faq;
  return meta;
});
const body = `export const BLOG_ARTICLES: BlogArticle[] = ${JSON.stringify(index, null, 2)};\n`;
...
for (const article of articles) {
  writeFileSync(
    join(CONTENT_DIR, `${article.slug}.${article.lang}.ts`),
    `${banner}import type { BlogFaqItem } from "@/lib/blog-data.generated";\n\n` +
      `export const faq: BlogFaqItem[] = ${JSON.stringify(article.faq)};\n\n` +
      `export const bodyHtml = ${JSON.stringify(article.bodyHtml)};\n`,
  );
}

console.log(
  `Generated ${articles.length} blog entries → ${OUT_FILE.replace(ROOT + "/", "")}` +
    ` + ${articles.length} content modules → ${CONTENT_DIR.replace(ROOT + "/", "")}/`,
);
